...
jQuery(document).ready(function($) {
  $(
    'link[href="https://ajax.googleapis.com/ajax/libs/jqueryui/1.7.2/themes/smoothness/jquery-ui.css"]'
  ).appendTo('head')

  $(
    'link[href="https://raw.githubusercontent.com/fus-marcom/fus-admissions-forms/events.css"]'
  ).appendTo('head')
  ...
  $('head').append(
    '<link rel="shortcut icon" href="https://franciscan.university/favicon.ico">'
  )
  ...
  $('h1')
    ...
    .next()
    .next()
    .css({ 'max-width': '70%', margin: '16px auto' })
  ...
  ;(function replaceTickets() {
    ...
    $inputArrPlus.map(function(i, ticketInput) {
      ...
      let ticketMinusName = $(ticketInput)
        .prev()
        .prev()
        .attr('name')
      ...
      if (ticketMinusName !== undefined) {
        $(ticketInput).after(
          `<input class="ticket-box" type="checkbox" name="ticket-box-${i}" checked="checked"/><label class="ticket-box-label" for="ticket-box-${i}" ticket-plus-name=${ticketName} />`
        )
      } else {
        $(ticketInput).after(
          `<input class="ticket-box" type="checkbox" name="ticket-box-${i}"/><label class="ticket-box-label" for="ticket-box-${i}" ticket-plus-name=${ticketName} ticket-minus-name=${ticketMinusName} />`
        )
      }
    })
  ...
  function switchArrowImagesToFontIcons() {
    ...
    $imgArrowIconsArr.map(function(i, imgArrowIcon) {
      ...
      $(imgArrowIcon).after(
        `<i id="arrow-icon-${i}" class="material-icons ${imgClasses}" onclick="toggleArrow('arrow-icon-${i}'); ${onClickVal}">${iconName}</i>`
      )
    })
  }
